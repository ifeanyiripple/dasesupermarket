      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1">
          ...
          {DEMO_CATEGORIES.map(cat => (
            <button
              key={cat.id}
              onClick={() => onChange({ category: cat.label, page: 1 })}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-left transition-all ${filters.category === cat.label ? "bg-[#1a5c38] text-white" : "text-gray-600 hover:bg-[#f0faf4]"}`}
            >
