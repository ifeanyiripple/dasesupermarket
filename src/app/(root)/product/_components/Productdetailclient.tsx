...
import {
  ...
  Check, ArrowLeft, Zap, Package, MapPin, ThermometerSun
} from "lucide-react"
import Link from "next/link"
import ProductCard, { type CardProduct, formatPrice } from "@/components/ProductCard"

// ── DB product shape (from getProduct router) ─────────────────────────────────
type ProductImage = {
  id:        string
  color:     string
  colorCode: string
...
type TabId = "description" | "details" | "ingredients"

// ── Info tabs sub-component ───────────────────────────────────────────────────
function ProductInfoTabs({ product }: { product: DBProduct }) {
  const [tab, setTab] = useState<TabId>("description")

  const tabs: { id: TabId; label: string; emoji: string }[] = [
    { id: "description", label: "Description", emoji: "📝" },
    { id: "details",     label: "Product Info", emoji: "📋" },
    ...
    <div className="border border-gray-100 rounded-2xl overflow-hidden bg-white shadow-sm">
      <div className="flex border-b border-gray-100">
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 py-3 text-xs md:text-sm font-semibold transition-all ${tab === t.id ? "text-[#1a5c38] border-b-2 border-[#1a5c38] bg-[#f0faf4]" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"}`}
          >
            <span className="hidden sm:inline mr-1">{t.emoji}</span>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="p-4 min-h-[120px]">
        <AnimatePresence mode="wait">
          {tab === "description" && (
            <motion.div key="desc" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-sm text-gray-600 leading-relaxed">
          ...
          {tab === "details" && (
            <motion.div key="details" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-4">
              {specRows.length > 0 ? (
                <div className="divide-y divide-gray-50">
                  {specRows.map(row => (
                    <div key={row.label} className="flex items-start gap-3 py-2.5">
                      <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400 w-32 flex-shrink-0 pt-0.5">
                        {row.label}
                      </span>
                ...
                <p className="text-sm text-gray-400 italic">No detailed product info available yet.</p>
              )}

              {product.keyFeatures && product.keyFeatures.length > 0 && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Key Features</p>
          ...
          {tab === "ingredients" && (
            <motion.div key="ingr" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-sm text-gray-600 leading-relaxed">
              {product.ingredients ? (
                <p>{product.ingredients}</p>
              ) : (
                <p className="text-gray-400 italic">No ingredients information available for this product.</p>
              )}
            </motion.div>
          )}
        </AnimatePresence>
...
export default function ProductDetailClient({ product }: Props) {
  ...
  // ── Fetch related products via react-query ────────────────────────────────
  ...
  // ── Use real reviews from DB, fall back to empty ──────────────────────────
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-14">
          ...
          >
            ...
            <div className="flex items-baseline gap-3 pb-3 border-b border-gray-100">
              <span className="text-2xl md:text-3xl font-extrabold text-[#1a5c38]">{formatPrice(product.price)}</span>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400">
                Total: <span className="font-bold text-gray-700">{formatPrice(product.price * quantity)}</span>
              </span>
            </div>
            ...
            <div className="flex gap-3">
              <motion.button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-bold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${added ? "bg-[#2d7a4f] text-white" : "bg-[#1a5c38] hover:bg-[#2d7a4f] text-white shadow-lg shadow-[#1a5c38]/20"}`}
              >
                <AnimatePresence mode="wait">
                  {added ? (
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white/95 backdrop-blur-sm border-t border-gray-100 shadow-xl px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={handleAddToCart}
            className={`flex items-center gap-2 px-7 py-3 rounded-2xl text-sm font-bold transition-all duration-300 ${added ? "bg-[#2d7a4f] text-white" : "bg-[#1a5c38] text-white"}`}
          >
