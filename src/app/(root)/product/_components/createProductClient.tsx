...
import {
  ...
  ChefHat, Trash2, Hotel, BedDouble, Users,
} from "lucide-react"
...
export const AddProductForm = () => {
  ...
  // ── Meat options state ─────────────────────────────────────────────────────
  const [meatOptions,  setMeatOptions]  = useState<MeatOptionDraft[]>([])
  const [meatInput,    setMeatInput]    = useState({ name: "", price: "" })
  ...
  const { mutate: createProduct, isPending: isProductPending } = useMutation({
    mutationFn: async (data: ProductForm) =>
      client.products.createProduct.$post(data),
    onSuccess: () => {
      productForm.reset()
  ...
  // ── Meat option helpers ────────────────────────────────────────────────────
  const addMeatOption = () => {
    const name  = meatInput.name.trim()
    const price = parseFloat(meatInput.price)
    if (!name || isNaN(price) || price < 0) return
    const newOption: MeatOptionDraft = {
      id: Math.random().toString(36).slice(2), name, price,
      isDefault: meatOptions.length === 0,
    }
    const updated = [...meatOptions, newOption]
    setMeatOptions(updated)
    setFoodValue("meatOptions", updated, { shouldValidate: true })
    setMeatInput({ name: "", price: "" })
  }
  const removeMeatOption = (id: string) => {
    const filtered = meatOptions.filter((m) => m.id !== id)
    if (filtered.length > 0 && !filtered.some((m) => m.isDefault)) {
      filtered[0] = { ...filtered[0], isDefault: true }
    }
    setMeatOptions(filtered)
    setFoodValue("meatOptions", filtered, { shouldValidate: true })
  }
  const setDefaultMeat = (id: string) => {
    const updated = meatOptions.map((m) => ({ ...m, isDefault: m.id === id }))
    setMeatOptions(updated)
    setFoodValue("meatOptions", updated, { shouldValidate: true })
  }
  const clearMeatOptions = () => {
    setMeatOptions([])
    setMeatInput({ name: "", price: "" })
    setFoodValue("meatOptions", [], { shouldValidate: true })
  }
  ...
  const isAnyRoomImageUploading = roomImageDrafts.some(
    (d) => !d.firebaseUrl && !d.error
  )
        ...
        <>
          ...
          <form onSubmit={handleProductSubmit(onProductSubmit)} className="flex flex-col gap-5">
                  ...
                  <div className="flex flex-col gap-1.5 mt-1">
                    {keyFeatures.map((f, idx) => (
                      <div key={idx} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/40 border border-border">
                        ...
                        <button type="button" onClick={() => removeFeature(idx)} className="text-muted-foreground hover:text-red-500 transition-colors flex-shrink-0"><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {categories.filter((item) => item.label !== "All").map((item) => (
                  <button key={item.label} type="button"
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                      category === item.label ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40 hover:text-foreground"
                    }`}
                  ...
                  {(() => {
                    ...
                    if (entry && !entry.firebaseUrl) return (
                      <div className="flex flex-col gap-2">
                        ...
                        {entry.error && (
                          <Button type="button" variant="outline" size="sm" className="text-red-500 mt-1 w-fit" onClick={() => fileInputRef.current?.click()}>Retry upload</Button>
                        )}
                      </div>
                    )
                    return (
            ...
            <Button type="submit" disabled={isPending || isAnyUploading} className="w-full">
              {isPending ? <><Loader2 size={16} className="animate-spin mr-2" /> Creating product...</> : <><Package size={16} className="mr-2" /> Add Product</>}
            </Button>
          </form>
        </>
      )}
          ...
          <form onSubmit={handleFoodSubmit(onFoodSubmit)} className="flex flex-col gap-5">
            ...
            <div className="border border-border rounded-xl p-4 flex flex-col gap-3 bg-muted/10">
              <div className="flex items-center justify-between">
                ...
                {meatOptions.length > 0 && (
                  <button type="button" onClick={clearMeatOptions} disabled={isPending}
              ...
              <p className="text-xs text-muted-foreground -mt-1">
                Add different protein choices with individual prices. The first option you add becomes the default.
              </p>
              <div className="flex gap-2">
                <Input placeholder="Protein name (e.g. Beef)" value={meatInput.name}
                  onChange={(e) => setMeatInput((prev) => ({ ...prev, name: e.target.value }))}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addMeatOption() } }}
                  disabled={isPending} className="flex-1" />
                <div className="relative flex-shrink-0 w-32">
                  ...
                  <Input type="number" placeholder="Price" value={meatInput.price}
                    onChange={(e) => setMeatInput((prev) => ({ ...prev, price: e.target.value }))}
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addMeatOption() } }}
                    disabled={isPending} className="pl-7" min="0" />
                </div>
                <Button type="button" variant="outline" onClick={addMeatOption}
                  disabled={isPending || !meatInput.name.trim() || !meatInput.price} className="flex-shrink-0">
                  <Plus size={14} className="mr-1" /> Add
                </Button>
              </div>
              {meatOptions.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  {meatOptions.map((option) => (
                    <div key={option.id}
                      className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg border transition-all ${
                        option.isDefault ? "border-orange-300 bg-orange-50" : "border-border bg-background"
                      }`}>
                      <button type="button" onClick={() => setDefaultMeat(option.id)} title="Set as default"
                        className={`w-4 h-4 rounded-full border-2 flex-shrink-0 transition-colors flex items-center justify-center ${
                          option.isDefault ? "border-orange-400 bg-orange-400" : "border-gray-300 hover:border-orange-300"
                        }`}>
                        {option.isDefault && <span className="w-1.5 h-1.5 rounded-full bg-white block" />}
                      </button>
                      <span className="text-sm font-semibold text-foreground flex-1">{option.name}</span>
                      {option.isDefault && <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wide flex-shrink-0">default</span>}
                      <span className="text-sm font-bold text-foreground flex-shrink-0">₦{option.price.toLocaleString()}</span>
                      <button type="button" onClick={() => removeMeatOption(option.id)} disabled={isPending}
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Click the circle next to any option to mark it as the default protein.
                  </p>
                </div>
              )}
            </div>
                ...
                {FOOD_BADGE_OPTIONS.map((b) => (
                  <button key={b} type="button"
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                      foodBadge === b ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:border-primary/40"
                    }`}>
                    {b}
                  </button>
                ))}
                ...
                {FOOD_CATEGORIES.map((cat) => (
                  <button key={cat} type="button"
                    ...
                    className={`px-3 py-2 rounded-xl border text-sm font-medium text-left transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                      foodCategory === cat ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40 hover:text-foreground"
                    }`}>
                    {cat}
                  </button>
                ))}
      ...
      {formMode === "room" && (
        <>
          ...
          <form onSubmit={handleRoomSubmit(onRoomSubmit)} className="flex flex-col gap-5">
            ...
            {/* Standard Amenities — read-only display */}
            ...
            <Button type="submit"
              disabled={isPending || isAnyRoomImageUploading}
