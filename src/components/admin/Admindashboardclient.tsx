...
type Tab = "overview" | "products" | "orders" | "users" | "reviews"
...
const BADGE_STYLES: Record<string, string> = {
  new: "bg-blue-100 text-blue-700", sale: "bg-red-100 text-red-700",
  hot: "bg-orange-100 text-orange-700", organic: "bg-[#f0faf4] text-[#1a5c38]",
}
...
export default function AdminDashboardClient({ data }: { data: AdminData }) {
  ...
  const { mutate: deleteProduct, isPending: isDeletingProduct } = useMutation({
    ...
    onSuccess:  () => { toast.success("Product deleted"); setDeletingProduct(null); queryClient.invalidateQueries({ queryKey: ["admin-products"] }) },
  ...
  // Flatten all reviews across products
  const allReviews = useMemo(() => {
    return products.flatMap(p => (p.reviews ?? []).map((r: any) => ({ ...r, productName: p.name, productId: p.id })))
  }, [products])
    ...
    <div className="min-h-screen bg-[#f7fdfb]">
      ...
      {(showProductForm || editingProduct) && (
        <ProductFormModal
          ...
          onSuccess={() => { setShowProductForm(false); setEditingProduct(null); window.location.reload() }}
        />
      )}
      {deletingProduct && (
        <DeleteConfirm
          title="Delete Product"
          description={`Are you sure you want to permanently delete "${deletingProduct.name}"? This cannot be undone.`}
      ...
      {deletingReview && (
        <DeleteConfirm
          title="Delete Review"
          description={`Delete this review by ${deletingReview.user?.name ?? "Unknown"}? This cannot be undone.`}
          onConfirm={() => deleteReview(deletingReview.id)}
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        {/* Tab nav */}
        <div className="flex flex-wrap gap-1.5 mb-6 bg-gray-100 rounded-2xl p-1.5">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === t.id ? "bg-[#1a5c38] text-white shadow-md" : "text-gray-400 hover:text-gray-600"}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { label: "Add Product", icon: <Plus size={14}/>, action: () => { setShowProductForm(true) } },
                      { label: "View Orders", icon: <ShoppingCart size={14}/>, action: () => setActiveTab("orders") },
                      { label: "Customers",   icon: <Users size={14}/>,       action: () => setActiveTab("users") },
                      { label: "Reviews",     icon: <Star size={14}/>,         action: () => setActiveTab("reviews") },
                    ].map(a => (
              ...
              <div>
                ...
                {filteredProducts.length === 0 ? (
                  <div className="text-center py-16 text-gray-400 text-sm bg-white border border-gray-100 rounded-2xl">
                    No products found.{" "}
                    <button onClick={() => setShowProductForm(true)} className="text-[#1a5c38] font-medium hover:underline">Add your first product</button>
                  </div>
                ) : (
                      ...
                      <div key={p.id} className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                        ...
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            ...
                            <ActionMenu items={[
                              { label: "Edit",   icon: <Pencil size={13}/>,  onClick: () => setEditingProduct(p) },
                              { label: "View",   icon: <Eye size={13}/>,     onClick: () => window.open(`/product/${p.id}`, "_blank") },
                              { label: "Delete", icon: <Trash2 size={13}/>,  onClick: () => setDeletingProduct(p), variant: "danger" },
                            ]} />
                          </div>
                ...
                <Table head={["Customer", "Product", "Rating", "Comment", "Date", ""]} empty={filteredReviews.length === 0}>
                  {filteredReviews.map((r: any) => (
                    <TR key={r.id}>
                      ...
                      <TD>
                        <button onClick={() => setDeletingReview(r)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors">
                          <Trash2 size={13} />
                        </button>
                      </TD>
                    </TR>
