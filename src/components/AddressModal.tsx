...
const inputCls =
  ...
  "focus:outline-none focus:ring-2 focus:ring-[#1a5c38]/25 focus:border-[#1a5c38] transition-all"
  ...
  return (
    <div>
      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">
        {label}
        {required && <span className="text-[#1a5c38] ml-0.5">*</span>}
      </label>
      {children}
    </div>
  )
...
export function AddressModal({ address, onClose, onSave, onDelete }: AddressModalProps) {
  const [form, setForm] = useState<AddressForm>({
    label:       address?.label       ?? "Home",
    fullName:    address?.fullName    ?? "",
    phoneNumber: address?.phoneNumber ?? "",
    state:       address?.state       ?? "Oyo",
    lga:         address?.lga         ?? "",
    town:        address?.town        ?? "",
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-all ${
                    form.label === key
                      ...
                      : "bg-white text-gray-600 border-gray-200 hover:border-[#1a5c38]/40 hover:text-[#1a5c38]"
                  }`}
                >
          ...
          <label className="flex items-center gap-3 cursor-pointer select-none">
            <div className="relative">
              ...
              />
              <div className="w-10 h-6 bg-gray-200 rounded-full peer-checked:bg-[#1a5c38] transition-colors" />
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full py-3 bg-[#1a5c38] hover:bg-[#2d7a4f] text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-60 shadow-lg shadow-[#1a5c38]/20"
          >
            {saving ? (
